...
!SUBROUTINE ODEDRIVER(f0,t0,tf,dt)
SUBROUTINE ODEDRIVER(func)
	IMPLICIT NONE
	INTEGER :: i,j,k,m,n,order
	real :: f0,t0,t,dt,tf,y0,x0,deriv,deriv2,func
	external deriv,deriv2
	real, dimension(:), allocatable :: y,dy,x
	type(state) :: curstate
	...
	allocate(y(order))
	allocate(dy(order))
!	allocate(curstate%y(order))
allocate(curstate%y(n))
...
!	do i=1,n
	...
	!y(x1) = y1
	! do n=1,nmax
	!y(x_n+1) = y(x(n)) + (x(n+1) - x(n)) * f(x(n), y(x(n)))
	!enddo
	
	write(*,*) 't=',curstate%t,'step=',curstate%step
	

	
	call EULEREXPLICIT(curstate,dt,deriv2)
	
!		dy(1) = 
!		dy(2) = 
!		y = y + dt * dty
	!	call EULEREXPLICIT(t,y,dt,derivs)
	
	!  t,y should be contained in a struct called STATE (of system)
!	call EULEREXPLICIT(curstate,dt,derivs)
	...
	write(*,*) 'x vals: ',curstate%x
	write(*,*) 'y vals: ',curstate%y
...
SUBROUTINE EULEREXPLICIT(curstate,dt,deriv)
	IMPLICIT NONE
	INTEGER :: i,j,k,n,m,nsize,msize
...
!	enddo
	...
	curstate%y(curstate%step)  = curstate%y(curstate%step-1)  + dt * deriv(curstate%x(curstate%step-1),curstate%y(curstate%step-1))
	curstate%x(curstate%step) = curstate%t
	
	write(*,*) 'curstate%y(curstate%step)',curstate%y(curstate%step),'curstate%step=',curstate%step
	write(*,*) 't = ',curstate%t,'deriv()',deriv(curstate%x(curstate%step-1),curstate%y(curstate%step-1))
	write(*,*) 'y(step)=',curstate%y(curstate%step),'t = ',curstate%t
