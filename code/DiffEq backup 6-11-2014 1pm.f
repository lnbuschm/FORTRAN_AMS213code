...
SUBROUTINE CRANK_NICHOLSON_2D_INIT(x,y,bigI,bigJ,dt,D,fg1,fg2,fg3,fg4,A)
	...
	do j=1,bigJ 
		do i=1,bigI  
			...
			A(k,k) = 1.0 + dt * D * (dxf + dyf) !-2.0 * (dxf + dyf)
...
SUBROUTINE CN_FTCS_FULL_SOLVER(x,y,bigI,bigJ,dt,D,fg1,fg2,fg3,fg4,f3a,f3b,A,u)
	...
	do i=1,bigI
		...
		!	C = f3a(x(i),y(j)) + f3b(x(i),y(j))
		!	write(*,*) 'C=',C
			
			R(k,1) = u(i,j,0) - u(i,j,0) * dt * D * (dxf + dyf) !  CN Contribution
		!	R(k,1) = R(k,1) + u(i,j,0) * ( 1.0 - 2 * dt * (-C) * (dxf + dyf) )   ! FTCS contribution
...
SUBROUTINE FTCS_2D_SOLVER(x,y,bigI,bigJ,dt,D,fg1,fg2,fg3,fg4,u)
	...
	do i=1,bigI
		...
		! Check for indexing to boundary conditions
			...
			! Check for indexing to boundary conditions
			if (i .eq. bigI) then
			!	u(i,j,1) = u(i,j,1) + dt * D * dxf * fg4(y(j))
				!R(k,1) = R(k,1) + dt * D * dxf * fg4(y(j))
			else 
			endif
				
			if (i .eq. 1) then
	!			u(i,j,1) = u(i,j,1) + dt * D * dxf  * fg3(y(j))
			!	R(k,1) = R(k,1) + dt * D * dxf  * fg3(y(j))
			else
			endif
			
			if (j .eq. bigJ) then
	!			u(i,j,1) = u(i,j,1) + dt * D * dyf  * fg2(x(i))
				!R(k,1) = R(k,1) + dt * D * dyf  * fg2(x(i))
			else 
			endif
			
			if (j .eq. 1) then
	!			u(i,j,1) = u(i,j,1) +  dt * D * dyf  * fg1(x(i))
				!R(k,1) = R(k,1) + dt * D * dyf  * fg1(x(i))
			else
			endif
		enddo
...
SUBROUTINE FTCS_1D_SOLVER(x,y,bigI,bigJ,dt,D,fg1,fg2,fg3,fg4,u)
	...
	do i=1,bigI
		...
		! Check for indexing to boundary conditions
			if (i .lt. bigI) then
				if (i .gt. 1) then
				!  u(i,inew) = r*u(i-1,iold) + diag*u(i,iold) + r*u(i+1,iold)
				 	u(i,j,1) = u(i,j,1) + dt * D * dxf * u(i+1,j,0)
				 	u(i,j,1) = u(i,j,1) + dt * D * dxf * u(i-1,j,0)
				else  ! if i=1
				!	u(i,j,1) = u(i,j,1) + dt * D * dxf * fg3(y(j))
					u(i,j,1) = u(i,j,1) + dt * D * dxf * 1.0 + dt * D * dxf * u(i+1,j,0) !  + (1.0-2.0*dt*D*dxf*u(i,j,0))   ! fg3(y(j))
				!	 u(1,inew) = r*ua + diag*u(1,iold) + r*u(2,iold)
				endif
			else  ! if i=bigI
			!	u(i,j,1) = u(i,j,1) + dt * D * dxf * fg4(y(j))
				u(i,j,1) = u(i,j,1) + dt * D * dxf * 0.0 + dt * D * dxf * u(i-1,j,0) !  + (1.0-2.0*dt*D*dxf*u(i,j,0))  ! fg4(y(j))
				! u(nmesh,inew) = r*u(nmesh-1,iold) + diag*u(nmesh,iold)  + r*ub
